...
| A29 | A ledger checkpoint and its recovery WAL represent one immutable transaction history; numeric generation is never treated as lineage without exact entry-prefix equality. The WAL remains complete because version 1 performs no cutover. | Checkpoint restoration, suffix-only balance application, idempotency preservation, and divergence detection. | Inject same-generation forks, higher-generation forks, stale pending files, a checkpoint ahead of the WAL, WAL-prefix mutation, and corruption with/without a recomputed CRC. Any accepted non-prefix history or second economic effect falsifies A29. |
...
- **High impact:** local command/event recovery, canonical-path writer exclusion,
  ...
  replication, matching/risk snapshots, and fenced ledger-checkpoint cutover
  remain prerequisites for bounded production recovery claims.
- **High impact:** entry-before-balance recovery, independent trial/replay audit,
  canonical checksummed ledger checkpoints, exact WAL-prefix proof, and suffix
  application are implemented; external reconciliation, corrections, retention,
  and checkpoint-bounded restart remain.
...
- **Medium impact opportunity:** redundant checkpoint balances plus retained
  ...
  correction workflows are still required for cross-system reconciliation.
