...
2. New and replacement quantities satisfy the configured lot increment and
   inclusive size bounds; submitted limit prices satisfy the signed tick grid
   and collar. Market-to-limit carries no submitted price.
...
13. Order identifiers cannot be reused after an accepted new order.
    ...
    - Market-to-limit accepts only GTC or GTD. After ordinary admission and
      before STP, it captures the current best executable opposite price,
      including a hidden-only best level, emits `MarketToLimitPriced`, and
      becomes an ordinary limit order at exactly that price. It cannot sweep a
      worse price. Any residual rests there under the submitted display policy.
      An empty opposite book and every IOC, minimum IOC, FOK, or post-only
      lifetime are sequenced nonmutating rejections and do not consume the
      order ID. Direct stop activation remains market-or-limit only.
    ...
    - Conditional stop-trigger-sweep preparation fills the ordinary move-only
      selection lease once with the exact bounded eligible side prefix. Buy
      priority is `(trigger ascending, priority sequence, OrderId)` and sell
      priority is `(trigger descending, priority sequence, OrderId)`. Every
      selected trigger-index entry is validated against its exact dormant
      state. The observation binds the prior optional and requested sourced
      references, positive maximum, complete dormant snapshots, exact count,
      checked `u128` leaves, and remaining eligible backlog. Accepted commit
      validates and drains those same IDs without a second trigger-prefix
      selection or sort. An empty eligible prefix invokes the predicate
      without a lease or selected-order output.
...
15. Reserve and fully hidden qualifiers are accepted only for a resting-capable
    submitted limit or market-to-limit order. A marketable GTC/GTD order may
    execute from its total incoming leaves; reserve peak and hidden publication
    policy apply to any residual that joins the book.
...
22. Every active order appears exactly once in an intrusive account/side list,
    ...
    - Conditional block-and-cancel preparation reuses the same selection and
      selected-state path, but binds it to the current revisioned account fence,
      requested action, and resulting blocked state in one
      `AccountControlSubmissionObservation`. Conditional enable observes the
      current fence and resulting enabled state without acquiring a selection
      lease or allocating selected-order output. Replay, core rejection, and
      coupled-risk rejection bypass observation and predicate execution.
    - Conditional transition-and-cancel preparation fills the ordinary all-
      order lease once and binds the current `TradingStateObservation`, target,
      action, resulting state, and complete canonical active-order states in
      one `TradingStateControlSubmissionObservation`. Accepted commit validates
      and consumes those IDs without another all-order scan or sort.
      Conditional transition observes only current and resulting instrument
      state, with no selection lease or selected-order output. Replay and core
      rejection bypass observation and predicate execution.
...
23. Each side stores executable occupied prices in one finitely bounded stable-
    ...
    - Market-to-limit capture reads this executable cache once before matching.
      The frozen price remains unchanged if cancel-resting STP removes the
      captured maker and exposes a worse opposite level.
...
24. FOK and minimum-quantity IOC inspection never mutate state or materialize
    ...
    price, contributing execution-price count, and termination; and does not
    mutate or allocate. It is execution economics only: admission, account
    controls, risk, fees, and projected resting-order cancellation side effects
    are outside the result.
    `ImmediateExecutionSubmission` closes that local observation-to-command
    gap for one canonical, fully displayed IOC market or limit order. One
    exclusive mutable shard call performs ordinary command preparation,
    coupled-risk authorization when present, the exact private quote, a
    caller-supplied acceptance predicate, and commit of that same preparation.
    Exact replay and core or risk rejection bypass the predicate and carry no
    current quote. Predicate decline or unwind drops the preparation before
    any identity, sequence, event, risk, book, or WAL mutation. Acceptance
    commits the quoted command without an intervening shard mutation. Durable
    variants run the predicate before appending the command and then retain the
    existing command-before-state-before-report protocol. The submission and
    outcome are typed local APIs over existing `NewOrder` and report encodings;
    they add no wire value or persisted callback policy.
    `try_immediate_execution_curve` materializes the same private path as one
    positive external-execution aggregate per contributing price in aggressor
    market order. Its first scan obtains the fixed-size quote and exact row
    count; one fallible request for exactly that row count precedes an identical
    second scan that populates caller-owned output. The allocator may grant
    more capacity. Each row reports price, quantity, and derived exact signed
    raw-price notional. Row totals equal the embedded quote, and its final row
    equals the quote's worst execution price. Self-trade decrement and unfilled
    quantity remain only in the quote. Allocation failure returns no partial
    curve; success does not mutate or reserve book state and adds no wire value.
    `try_submit_immediate_execution_curve_if` binds that complete caller-owned
    curve to the A140 canonical IOC decision under the same exclusive mutable
    shard borrow. It reuses A140 preparation and its already computed quote,
    requests the exact curve row count, then performs only the identical
    population scan before invoking a predicate that borrows the curve. Exact
    replay and core or coupled-risk rejection bypass curve construction and the
    predicate. Allocation failure, predicate decline, or unwind precedes
    identity, sequence, risk, book, and WAL mutation. Acceptance commits the
    same preparation and retains the exact curve with its report. Durable plain
    and coupled-risk variants run curve construction and the predicate before
    command append, then reuse the existing command-before-state-before-report
    protocol. The operation introduces no new command, report, or wire value.
    `submit_new_order_if` and `try_submit_new_order_curve_if` generalize that
    atomic local decision boundary to every `NewOrder` shape without adding a
    second preparation or matching path. One `OrderExecution<T>` value is
    either `Active(T)`, carrying the A124 quote or A141 curve, or
    `DormantStop`, carrying no forecast of activation-time liquidity. Exact
    replay plus core or coupled-risk rejection bypass observation construction
    and the predicate. A valid noncrossing post-only order is active with an
    empty observation; a crossing post-only order and an insufficient FOK are
    core rejections and bypass the predicate. Market-to-limit preparation and
    commit read the same private executable best under one exclusive borrow,
    so the observation uses the exact limit captured by the pricing event.
    The observation reports current A124 execution economics; submitted TIF
    remains authoritative. In particular, an accepted minimum-quantity IOC can
    observe positive sub-threshold liquidity and then atomically cancel its
    complete quantity without trading. Curve allocation failure, predicate
    decline, or unwind precedes identity, sequence, risk, book, and WAL
    mutation. Durable acceptance and business rejection retain two existing
    frames; allocation failure, decline, unwind, and replay append zero. No
    command, report, snapshot, market-data, or wire value changes.
    `submit_replace_order_if` and `try_submit_replace_order_curve_if` apply the
    same atomic local boundary to an existing continuous order. Ordinary
    replacement preparation first fixes replay, ownership, admission, display-
    mode, dormant-stop, capacity, sequence, and core-rejection results. For an
    active target, `OrderExecution<T>::Active` maps the target's immutable
    account, side, and STP policy plus the replacement quantity and price to
    the A124 quote or A141 curve. The target is same-side state, so retaining it
    during observation cannot alter the scanned opposite liquidity. A dormant
    stop-limit yields `DormantStop` without an activation-time scan; a dormant
    stop-market replacement remains a core rejection. Coupled risk applies its
    existing net-replacement authorization before observation and bypasses the
    predicate on rejection. Decline, unwind, and curve-allocation failure leave
    the target and every coupled reservation unchanged. Durable acceptance and
    business rejection retain the existing two frames; decline, unwind,
    allocation failure, and replay append zero. The API reuses existing
    `ReplaceOrder`, report, checkpoint, market-data, and wire values.
    `submit_cancel_order_if` composes ordinary `CancelOrder` preparation with
    one `ActiveOrderObservation`, a borrowed local predicate, and commit of
    that same preparation. The observation binds the instrument ID,
    definition version, last visible book event sequence, queried order ID,
    and validated resting or dormant-stop state. Exact replay and core
    rejection bypass observation and predicate execution. Observation failure,
    predicate decline, or unwind precedes sequence, event, matching, risk,
    history, and WAL mutation. Acceptance uses the ordinary cancellation path;
    ...
    no command, report, snapshot, market-data, or wire value.
    `try_submit_account_control_if` composes ordinary `AccountControl`
    preparation with one `AccountControlSubmissionObservation`, a borrowed
    local predicate, and commit of that same preparation. The observation binds
    the current `AccountControlObservation`, requested action, resulting fence,
    and, for block-and-cancel, the exact ascending selected states, count, and
    checked `u128` total leaves. Acceptance consumes the same prepared IDs;
    query failure, predicate decline, or unwind returns the selection lease and
    changes no matching, risk, history, or WAL state. Enable has empty selected
    state and consumes no lease. Coupled risk rejects an unprofiled account
    before selected-state construction or predicate execution. Durable
    ...
    `TradingStateControl` preparation with one
    `TradingStateControlSubmissionObservation`, a borrowed local predicate,
    and commit of that same preparation. The observation binds the current
    `TradingStateObservation`, requested target and action, resulting state,
    and, for transition-and-cancel, every ascending active resting and dormant-
    stop state, exact count, and checked `u128` total leaves. Acceptance
    validates and consumes the same prepared IDs without another all-order
    scan or sort; query failure, predicate decline, or unwind returns the lease
    and changes no matching, risk, history, or WAL state. Transition has empty
    selected state and consumes no lease. Coupled risk performs its account-
    ...
    preparation with one `StopTriggerSweepObservation`, a borrowed local
    predicate, and commit of that same preparation. The observation binds
    instrument, definition version, event sequence, prior optional reference,
    requested sourced reference, positive maximum, exact canonical eligible
    dormant-stop states, checked `u128` total leaves, and remaining eligible
    count. Selection fills the ordinary trigger lease once; accepted commit
    validates and drains those IDs without another trigger-prefix selection or
    sort. A valid empty prefix invokes the predicate without a lease or
    selected-order output. Replay and core rejection bypass selection, output,
    and predicate; coupled risk performs its account-independent trigger-sweep
    authorization before observation. Query failure, decline, or unwind
    changes no matching, risk, reference, history, or WAL state. Durable
    acceptance and business rejection use the existing two frames; query
    failure, decline, unwind, and replay append zero. No wire value changes.
    ...
    unquoted lots, signed raw-price notional, worst quoted price, contributing
    price count, and termination, and shares the private quote's checked
    notional accumulator. It is a public-depth observation rather than a
    private execution prediction or liquidity reservation.
...
27. Construction fallibly reserves four price AVL arenas—execution and public
    ...
    - `PreparedCommand` owns an optional lease for a non-empty
      mass-cancel/expiry/stop-trigger/account-control/instrument-control
      selection;
...
29. A GTC/GTD/post-only capacity preview is invoked only when an active-order,
    ...
    - Accepted market-to-limit capacity prediction uses its available
      executable best as the effective limit. If a residual would create an
      absent same-side level at the bound, preparation returns the typed side-
      level capacity error before acceptance, trade, STP, sequence, or identity
      mutation.
...
31. An account without retained administrative state is enabled at revision
    ...
    - Conditional account control observes the exact current fence, action,
      resulting state, and block-and-cancel order set inside the same exclusive
      borrow that commits the prepared command. It therefore closes the local
      query-to-control race without creating a remotely valid fence token.
      Enable has no selected-order storage; block-and-cancel reuses the ordinary
      constructor-owned selection lease and cancellation trace.
32. Effective instrument trading state starts from the immutable definition at
    ...
    - Conditional state control observes the exact current state/revision,
      target, action, and resulting state inside the same exclusive borrow that
      commits the prepared command. Transition-and-cancel additionally exposes
      and consumes one canonical complete active-order selection; transition-
      only allocates no selected-order output and acquires no selection lease.
...
34. Every resting order contributes mutation-maintained future event work.
    ...
    - Preparation combines these aggregates with the incoming quantity in
      ...
      to obtain a safe `O(1)` event and trade bound. Market-to-limit adds its
      one mandatory pricing event to the ordinary effective-limit bound.
...
independently to bids and asks in market priority. Its fixed-size
`PublicDepthImbalance` binds both selected prefixes to one instrument,
definition version, and event sequence; retains per-side level/order/quantity
totals and best/worst prices; and represents normalized displayed-quantity
imbalance for non-empty depth exactly as side plus absolute numerator over
checked combined quantity. Equal positive prefixes and the empty selection
both have no imbalance side; only the empty selection has a zero denominator
and therefore no normalized ratio. Fully hidden prices and reserve-hidden
leaves are absent. The query first validates coherent public extrema, then
folds public-depth candidates through the same private
`DepthAggregateTotals` checked accumulator used by `DepthSummary`. For `K_b`
and `K_a` occupied execution prices traversed to select at most `N` visible
levels per side, it costs `O(log(P + 1) + K_b + K_a)` time and `O(1)` space;
hidden-only prices can contribute to traversal but not output. Failure discards
the complete local value, and the successful path allocates and mutates
nothing.
...
fully hidden-only prices remain absent. A selected cached side with zero public
quantity/count or a locked/crossed pair fails with a typed invariant error.
For a two-sided quote, exact raw spread uses `u64` and the exact midpoint
...
`try_resolve_pegged_price` composes that BBO with immutable tick/collar rules
to select a displayed primary, market, or explicitly rounded midpoint
reference. It applies one signed tick offset, optional buy cap or sell floor,
and allow/reject/passive-slide crossing policy in fixed-size checked integer
state. `PeggedPriceResolution` retains the complete BBO provenance plus its
request, intermediate prices, final limit, and slide status. Resolution is
`O(1)`, allocation-free, and nonmutating.
`submit_pegged_new_order_if` performs ordinary new-order preparation, then
requires one new admissible limit command and the current complete BBO to match
that retained resolution before invoking its process-local predicate. Exact
replay and core/risk rejection retain their ordinary bypass semantics.
Acceptance commits and, through durable wrappers, persists only the existing
ordinary limit command/report; no persistent peg or automatic repricing state
is introduced.
...
provenance, so absence is state-bound rather than an unversioned `None`.
Fully hidden-only prices are absent and reserve orders expose only current
displayed slices. At the requested key, execution-level occupancy and the
redundant public-price AVL membership must agree; a candidate row must have
positive displayed quantity/count and its embedded price must equal the key.
Off-grid or out-of-collar raw prices remain valid lookup keys and ordinarily
return absence; this read-only query does not perform order admission. Exact
lookup is `O(log(P + 1))` time with `O(1)` fixed state and no successful-path
allocation or mutation. The `level` convenience query delegates to this typed
path and panics on corruption rather than silently returning absence.
...
contributing price count, and filled/price-limit/book-exhausted termination.
Every inspected candidate must have positive public quantity/count. Fully
hidden prices are skipped; reserve orders contribute only their current
visible slices. The shared fixed-size fold and checked notional accumulator are
also used by the private quote. A successful query allocates nothing and does
not mutate or reserve book state. Human-readable invariant detail may allocate
only after corrupt public aggregates are detected. For `K` occupied execution
prices traversed through termination among `P` prices, work is
`O(log(P + 1) + K)` time and `O(1)` space.
...
`try_active_order_observation` performs one expected constant-time private
identity lookup and returns one fixed-size `ActiveOrderObservation`. The value
binds the queried `OrderId`, instrument, immutable definition version, and
final event sequence to absent, resting, or dormant-stop state through
`ActiveOrderSnapshot`. State-bound absence therefore cannot be confused with
an unversioned lookup miss. A present key must equal the embedded order
identity. Resting state validates positive total and working quantities plus
the display relationship; dormant state validates positive leaves, canonical
working quantity, activation risk price, absence of price-level FIFO links,
non-zero trigger priority, and lifetime/expiry agreement. The successful path
allocates nothing and does not mutate the book. `try_order` and
`try_dormant_stop` are typed projections; `order` and `dormant_stop` delegate
to them and panic on selected-row corruption. The observation does not prove
the complete price, account, expiry, or trigger-index topology; queue-position,
price-level, and `OrderBook::validate` queries retain those stronger scopes.

`try_order_queue_position` returns another fixed-size private value for one
resting `OrderId`. For a displayed-class target it sums only current working
slices of prior displayed orders because their later reserve refreshes requeue
behind the target. For a hidden target it sums every total leaf in the prior
displayed class and every earlier hidden leaf because displayed refreshes
remain ahead of hidden liquidity. The result binds the complete order snapshot
to instrument/version/event-sequence provenance, counts distinct orders and
exact executable lots ahead of the target's current slice, and returns a typed
invariant failure if the relevant predecessor path contradicts side, price,
class, identity, or bidirectional FIFO links. Missing and dormant identities
return absence. Successful traversal allocates nothing and does not predict
priority after the target's current slice refreshes.

`try_price_level_orders` instead validates one complete private `(side, price)`
queue before returning output. The shared level checker proves active identity,
side/price membership, non-dormant valid working/total state, reciprocal FIFO,
displayed/reserve-before-hidden class order, head/tail/count, displayed tail,
public aggregate, and future-event work. `PriceLevelOrders` then exposes an
allocation-free, exact-size, fused, double-ended stream of complete
`OrderSnapshot` values in executable priority. An absent level is empty; a
relevant contradiction is a typed failure before any order is exposed. The
immutable book borrow prevents safe mutation between validation and traversal.
For `K` level members among `P` occupied prices, validation plus complete
iteration is expected `O(log(P + 1) + K)` time with `O(1)` iterator state.
...
`OrderId` elements. Non-empty mass-cancel, expiry-sweep, stop-trigger-sweep,
block-and-cancel, or transition-and-cancel preparation leases one vector;
zero-cardinality selection bypasses the pool. Read-only pool telemetry exposes
configured, available, and per-lease allocated cardinalities.
...
observation is constructed. Conditional block-and-cancel uses the same path,
while conditional enable acquires no lease and constructs no selected-order
output. Accepted commit drains the same prepared IDs. RAII returns the buffer
on every completion or rejection path. Commit adds
expected `O(1)` identity, idempotency, and generation validation before the
already-prepared transition.

Conditional transition-and-cancel likewise fills and sorts the all-order lease
once before constructing its caller-owned observation; conditional transition
...
Conditional stop-trigger-sweep preparation fills the ordinary lease with the
exact canonical eligible prefix, validates each selected dormant row through
its exact side trigger-index entry, and constructs exactly reserved caller
output. Accepted commit validates and drains those same IDs without a second
prefix selection or sort. For `K` selected stops and `S` dormant stops in the
selected side index, the added exact-index work is `O(K log(S + 1))`, leased ID
scratch and caller output are each `O(K)`, and an empty prefix uses neither
resource.
...
12. For `M <= O` affected orders, commit is
    ...
    - The book primitive itself has no command sequence, phase, idempotency,
      ...
      and the version-20 durable wrapper supplies stable wire encoding,
      full-WAL recovery, and snapshot-version-20 checkpoint/cutover recovery.
...
15. `try_submit_order_if` holds one exclusive mutable engine borrow across
    ordinary admission preparation, exact predicted-state validation, one
    synchronous predicate, and commit of that same-generation preparation.
    - `CallAuctionSubmitObservation` is a fixed-size owned value carrying the
      exact priority-bearing state acceptance will admit, command/time,
      instrument/version, prospective command and first-event sequences,
      phase/cycle, source/resulting book revisions, and the previous still-
      current indication.
    - Before invoking the predicate, the engine rejects foreign or stale engine
      and book generations, repeats admission preflight, proves the candidate
      identity absent, and validates phase/cycle/indication coherence. Exact
      replay plus core or coupled-risk business rejection bypasses observation
      and predicate and follows the ordinary report path.
    - Decline returns the owned observation and unwind drops it; neither path
      consumes identity, priority, sequence, indication, book, history, risk,
      or durable state. Acceptance revalidates the exact predicted state,
      admits it at one successor revision, emits one equal acceptance event,
      invalidates the indication, and inserts its coupled reservation once.
      The observation and decision are process-local and add no encoded value.
16. `try_submit_indicative_if` holds one exclusive mutable engine borrow across
    ordinary aggregate reconstruction/discovery, one synchronous predicate,
    and publication of that same-generation prepared state.
    - `CallAuctionIndicativeObservation` is a fixed-size owned value carrying
      exact previous and prepared nullable indications, command/time,
      instrument/version, prospective command and publication-event sequences,
      phase/cycle, and the unchanged source/resulting book revision.
    - Before invoking the predicate, the engine rejects foreign or stale
      engine, history, book, or phase generations. One shared validator proves
      command/state coordinates, phase/cycle lineage, structural price
      validity, and prior-indication coherence both before the predicate and at
      accepted commit. Replay and core business rejection bypass observation
      and predicate.
    - Decline returns the owned observation and unwind drops it; neither path
      changes sequence, event, latest indication, book, history, risk, or WAL.
...
8. The coupled checkpoint has a stable complete-value little-endian codec and
   direct restore under default or explicit limits. Snapshot version 20 assigns
...
1. WAL version 20 retains stable record-kind tags `9` and `10` for one
...
2. An uncut auction journal contains one immutable instrument definition
   ...
   with one kind-`8` anchor bound to snapshot-version-20 call-auction kind `4`,
...
4. Incoming notional covers every reachable execution price: buy limits use
   ...
   trigger threshold. Market-to-limit is authorized against the full market
   collar before its price is captured. Units are raw price quanta multiplied
   by lots (`u128`).
...
6. Once an order rests or becomes dormant, its reservation retains the
   ...
   current total leaves. A market-to-limit residual uses its captured limit,
   not the pre-acceptance market constraint, for this reservation.
...
16. `RiskManagedLimits` requires matching account-control capacity to be at
    ...
    Conditional account-control preparation performs that profile authorization
    before constructing selected-order output or invoking its predicate.
    ...
    Stop-trigger control is account-independent; conditional trigger-sweep
    preparation performs that existing authorization before constructing
    dormant-stop output or invoking its predicate. Accepted commit transitions
    exactly the observed reservations through ordinary activation, execution,
    cancellation, and residual reservation application. Noncommit changes no
    reservation, exposure, or position.
...
6. The publisher tracks resting order side, price, total leaves, executable
   ...
   For market-to-limit, it independently scans its complete private order
   mirror, including hidden-only levels, and requires exactly one canonical
   pricing event before any aggressor trade, STP interaction, or residual. All
   such interactions must use that frozen price; the pricing event projects to
   `NoBookChange`.
...
20. Snapshot application validates identity, staleness, grammar, and both side
    ...
    invalid aggregates, locked/crossed extrema, or cumulative overflow return
    typed source divergence without output or mutation. Only an explicit-band
    summary is exposed because the replica owns no definition price endpoints.
    `try_public_depth_imbalance` reuses the authoritative fixed-size value and
    the same per-side checked accumulator, applies one level limit independently
    to the two public-only AVL traversals, and retains exact replica provenance.
    The shared observation gate rejects poison and incoherent extrema before
    either fold. Selected-row contradiction, per-side overflow, or combined
    bid-plus-ask overflow maps to typed source divergence without partial
    output. For `B` bid levels, `A` ask levels, and limit `N`, the post-gate fold
    is `O(min(B, N) + min(A, N))` time and `O(1)` space. A healthy caught-up
    replica returns the exact authoritative value.
    `try_public_level` reuses the authoritative fixed-size observation and
    selected-row validator after the shared poison/coherent-extrema gate. One
    exact public-AVL lookup retains present or absent state with replica
    provenance; target-row corruption maps to typed source divergence. The
    convenience `level` query composes the fallible path and panics on unhealthy
    state. A caught-up healthy replica equals the authoritative observation at
    the same side and price in `O(log(P + 1))` time and `O(1)` space.
    `try_displayed_liquidity_quote` reuses the authoritative fixed-size public-
...
22. Matching, coupled risk, ledger, and call-auction cutover publishes an
    inactive A/B checkpoint slot before publishing a synchronized version-20
    anchor and any
...
1. A version-20 `QSNP` file carries a fixed 28 B header with magic, typed
...
9. Ledger, matching, coupled risk, and call-auction checkpoints retain complete
   ...
   to one version-20 A/B snapshot slot, so reopen scans only the
...
[WAL format version 20](wal-v20.md) and
[Semantic snapshot format version 20](snapshot-v20.md). Filesystem and device
...
Active-order observation tests cover resting, dormant, and state-bound absent
results, every selected-row validation class, typed projections, direct
checkpoint restoration, dormant-to-resting activation, and full-WAL recovery.
...
bands, exact selected-row materialization, and hidden-only exclusion. Checked
depth-summary tests cover both market directions, full and inclusive bands,
exact endpoints, inversion, hidden-only exclusion, aggregate totals,
best/worst prices, provenance, nonmutation, direct checkpoint restoration, and
typed zero-aggregate/count plus cumulative-overflow corruption. BBO tests cover
empty, one-sided, hidden-only, and two-sided public state; instrument/
version/event-sequence provenance; nonmutation; exact full-domain spread and
half-unit midpoint arithmetic; direct checkpoint restoration; and typed zero-
aggregate/count plus locked/crossed cache corruption. Live allocation tests
invert class and arrival order, place the class boundary at
price-time and
...
Conditional call-auction submission tests cover fixed-size owned command,
sequence, phase, book, prior-indication, and exact predicted priority-bearing
state provenance; absent identity and stale-generation rejection before the
predicate; acceptance, decline, and unwind; core/risk-rejection and replay
predicate bypass; exact admitted event/state and coupled reservation insertion;
zero-frame durable noncommit, two-frame acceptance/business rejection, and
exact plain/coupled-risk reopen state.
Conditional call-auction indicative tests cover fixed-size owned command,
sequence, collecting/frozen phase, book, previous-indication, band, reference,
policy, and exact nullable clearing provenance; book/phase staleness before the
predicate and book staleness at commit; acceptance, decline, and unwind; core-
...
The same 20,000 generated books compare every private execution-curve price and
quantity with that independent model. Direct tests cover exact quote/row sum,
notional, count, worst-price, committed-trace parity, reserve and fully hidden
liquidity, cancel-resting, decrement-and-cancel self consumption, both market
directions, nonmutation, and typed output-resource attribution.
Conditional immediate-execution tests bind the typed submission to its
canonical fully displayed IOC command and require exact quote/committed-trace
parity. They cover predicate acceptance, decline, and unwind; core and coupled-
risk rejection and exact-replay predicate bypass; unchanged identity,
sequence, book, and risk state on noncommit paths; zero WAL growth on decline,
unwind, and replay; accepted command/report framing; and plain and coupled-risk
durable reopen.
Curve-aware conditional tests additionally require exact per-price
curve/committed-trace parity, typed curve ownership on decline and acceptance,
curve-construction and predicate bypass on core/risk rejection and replay,
zero WAL growth on decline and unwind, coupled position parity, and exact plain
and coupled-risk durable recovery.
All-new-order conditional tests cover market, market-to-limit, limit, GTC,
GTD, IOC, minimum-quantity IOC, FOK, post-only, fully displayed, reserve,
fully hidden, and dormant-stop shapes through the shared quote/curve path.
They require explicit active versus dormant observations, frozen private-best
market-to-limit parity, empty valid post-only observation, predicate bypass on
core/risk rejection and replay, unchanged identity/sequence/book/risk/WAL state
on decline or unwind, two-frame durable acceptance, zero-frame replay, exact
residual reservations and positions, and plain/coupled-risk reopen parity.
Conditional-replacement tests cover active price-changing quote/curve binding,
exact per-price committed-trace parity, priority-retaining same-price reduction,
explicit dormant stop-limit state, predicate decline and unwind, core/risk
rejection and exact-replay predicate bypass, unchanged target/reservation on
all nonacceptance paths, zero WAL growth on decline, unwind, and replay, two-
frame durable acceptance and business rejection, and plain/coupled-risk reopen
parity.
Conditional-cancellation tests cover validated resting and dormant-stop
observations; predicate acceptance, decline, and unwind; unknown-order and
wrong-owner rejection plus exact-replay predicate bypass; unchanged target and
reservation on noncommit; reservation release on acceptance; zero durable WAL
...
Conditional account-control tests cover current fence/action/resulting-state
provenance; mixed fully displayed, reserve, fully hidden, and dormant-stop
block-and-cancel selections; enable without selected output or a lease;
predicate acceptance, decline, and unwind; wrong-version, stale-revision,
unprofiled-risk, and exact-replay predicate bypass; coupled reservation release;
zero-frame durable noncommit and replay; two-frame business rejection and
acceptance; and plain/coupled-risk reopen parity. An internal account-list
corruption test requires a typed query failure before predicate execution or
semantic mutation and proves the selection lease is returned.
Conditional trading-state-control tests cover exact current state/revision,
target/action/resulting-state provenance; empty and mixed fully displayed,
reserve, fully hidden, and dormant-stop transition-and-cancel selections;
transition-only without selected output or a lease; predicate acceptance,
decline, and unwind; wrong-version, stale-revision, unchanged-state,
transition-and-cancel-to-open, and exact-replay predicate bypass; coupled
reservation release; zero-frame durable noncommit and replay; two-frame
business rejection and acceptance; and plain/coupled-risk reopen parity. An
...
Conditional stop-trigger-sweep tests cover prior/requested sourced-reference
provenance; empty, bounded, and remaining-backlog prefixes; equal triggers with
nonmonotonic IDs; canonical buy and sell trigger/priority/ID order; complete
dormant snapshots; exact count and `u128` leaves; predicate acceptance,
decline, and unwind; wrong-version, discontinuous reference, unfinished-backlog
advance, and exact-replay predicate bypass; coupled reservation transitions;
zero-frame durable noncommit and replay; two-frame business rejection and
acceptance; and plain/coupled-risk reopen parity. An internal trigger-index
corruption test requires a typed query failure before predicate execution or
semantic mutation and proves the selection lease is returned.
...
exclusion, signed and zero-adjacent prices, `i64`/`u64` extrema, exact notional
and price counts, nonmutation, poisoned/corrupt replicas, and
authoritative/replica parity across incremental updates, retry, snapshot
repair, and durable publisher bootstrap.
Queue-position tests cover displayed, reserve, and fully hidden targets before
and after reserve refresh, exact current-slice versus total-leaves accounting,
missing identities, provenance, and nonmutation. An independent forward FIFO
model differentially checks every resting order across 20,000 generated books;
white-box corruption breaks one predecessor's next link and requires a typed
failure.
Private price-level iterator tests cover displayed, reserve, and hidden class
order; hidden-only and absent prices; exact-size mixed forward/reverse
consumption; current slices and reserve refresh to the displayed-class tail;
nonmutation; direct checkpoint and full-WAL restoration; and typed pre-output
link corruption. Every level in the same 20,000 generated books is compared
in both directions with an independent forward-link model.
