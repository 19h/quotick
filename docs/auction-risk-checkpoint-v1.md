...
and the payload retained by snapshot-version-7 `QSNP` kind `5`. It has no WAL
...
| Order | Width | Field |
|---:|---:|---|
| 1 | variable | account-risk-definition length `u32`, then its stable WAL-v7 kind-5 payload without a WAL header |
...
- `SnapshotFile` supplies version-7 framing, CRC protection, synchronized
