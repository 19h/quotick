...
/// to instrument, definition-version, and book-event-sequence provenance.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl DepthSummary {
    ...
    /// Returns the summarized side.
    #[must_use]
    pub const fn side(self) -> Side {
        self.side
    }

    /// Returns the caller's inclusive range start.
    #[must_use]
    pub const fn range_start(self) -> Price {
        self.range_start
    }

    /// Returns the caller's inclusive range end.
    #[must_use]
    pub const fn range_end(self) -> Price {
        self.range_end
    }
...
pub enum OrderBookQueryResource {
    ...
    MassCancelOrders,
    /// Complete active-order states selected by one conditional account control.
...
impl<T> OrderExecution<T> {
    ...
    pub const fn active(&self) -> Option<&T> {
        match self {
            ...
            Self::DormantStop => None,
        }
    }

    /// Returns whether this observation represents a dormant stop.
    #[must_use]
    pub const fn is_dormant_stop(&self) -> bool {
        matches!(self, Self::DormantStop)
    }
}
...
#[derive(Debug)]
pub(crate) struct ConditionalExecutionPreflight<T> {
    pub(crate) preparation: CommandPreparation,
    pub(crate) observation: Option<T>,
}
...
pub(crate) fn evaluate_conditional_execution<T, U, E>(
    preflight: ConditionalExecutionPreflight<T>,
    observe: impl FnOnce(T) -> Result<U, E>,
    ...
    match preflight.preparation {
        ...
        CommandPreparation::Ready(prepared) => {
            let observation = match preflight.observation {
                Some(value) => Some(observe(value)?),
                None => None,
            };
            ...
            Ok(ConditionalExecutionPreparation::Commit {
                prepared,
                observation,
            })
        }
...
fn try_account_order_selection_quantity(
    account_id: AccountId,
    scope: MassCancelScope,
    ...
    for order in orders {
        if order.account_id() != account_id {
            return Err(InvariantViolation::new(format!(
                "account {account_id} selection contains order {} owned by {}",
                order.order_id(),
                order.account_id()
            )));
        }
        ...
        {
            return Err(InvariantViolation::new(format!(
                "account {account_id} {side:?} selection contains {:?} order {}",
                order.side(),
                order.order_id()
            )));
        }
    }
    try_order_selection_quantity(orders, "account")
}

fn try_order_selection_quantity(
    orders: &[ActiveOrderSnapshot],
...
fn try_order_selection_quantity_by_key<K: Ord>(
    ...
    for order in orders {
        ...
        selected_quantity_lots = selected_quantity_lots
            ...
            .ok_or_else(|| {
                InvariantViolation::new(format!("selected {selection} quantity is exhausted"))
            })?;
    }
    Ok(selected_quantity_lots)
}
...
impl MassCancelObservation {
    fn try_new(
        ...
        let selected_quantity_lots =
            try_account_order_selection_quantity(account_id, scope, &orders)?;
        Ok(Self {
            instrument_id,
            instrument_version,
            book_event_sequence,
...
/// Frozen current fence and private state selected by one account control.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AccountControlSubmissionObservation {
    control: AccountControlObservation,
    action: AccountControlAction,
    orders: Vec<ActiveOrderSnapshot>,
    selected_quantity_lots: u128,
}

impl AccountControlSubmissionObservation {
    fn try_new(
        control: AccountControlObservation,
        action: AccountControlAction,
        orders: Vec<ActiveOrderSnapshot>,
    ) -> Result<Self, InvariantViolation> {
        if action == AccountControlAction::Enable && !orders.is_empty() {
            return Err(InvariantViolation::new(
                "account enable observation contains selected orders",
            ));
        }
        if control.state() == AccountAdmissionState::Blocked && !orders.is_empty() {
            return Err(InvariantViolation::new(
                "blocked account-control observation contains active orders",
            ));
        }
        let selected_quantity_lots = try_account_order_selection_quantity(
            control.account_id(),
            MassCancelScope::All,
            &orders,
        )?;
    ...
    #[must_use]
    pub const fn instrument_id(&self) -> InstrumentId {
        self.control.instrument_id()
    }

    /// Returns the immutable instrument-definition version used by preparation.
    #[must_use]
    pub const fn instrument_version(&self) -> InstrumentVersion {
        self.control.instrument_version()
    }

    /// Returns the last committed event sequence visible to preparation.
    #[must_use]
    pub const fn book_event_sequence(&self) -> u64 {
        self.control.book_event_sequence()
    }

    /// Returns the account selected by the control.
    #[must_use]
    pub const fn account_id(&self) -> AccountId {
        self.control.account_id()
    }

    /// Returns the frozen current account-fence observation.
    #[must_use]
    pub const fn control(&self) -> AccountControlObservation {
        self.control
    }

    /// Returns the requested account-control action.
    #[must_use]
    pub const fn action(&self) -> AccountControlAction {
        self.action
    }

    /// Returns the admission state produced by the requested action.
    #[must_use]
    pub const fn resulting_state(&self) -> AccountAdmissionState {
        self.action.resulting_state()
    }

    /// Returns complete selected states in ascending order-identifier order.
    ///
    /// Enable observations are empty. Block-and-cancel observations contain
    /// every active resting or dormant-stop order for the controlled account.
    #[must_use]
    pub fn orders(&self) -> &[ActiveOrderSnapshot] {
        &self.orders
    }

    /// Returns the number of active orders selected for cancellation.
    #[must_use]
    pub fn selected_order_count(&self) -> usize {
        self.orders.len()
    }
...
/// Frozen current state and private orders selected by one trading-state control.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TradingStateControlSubmissionObservation {
    control: TradingStateObservation,
...
impl TradingStateControlSubmissionObservation {
    fn try_new(
        control: TradingStateObservation,
        target_state: TradingState,
        action: TradingStateControlAction,
        ...
        if control.state() == target_state {
            return Err(InvariantViolation::new(
                "trading-state-control observation has an unchanged target state",
            ));
        }
        if action == TradingStateControlAction::Transition && !orders.is_empty() {
            return Err(InvariantViolation::new(
                "transition-only trading-state observation contains selected orders",
            ));
        }
        if action == TradingStateControlAction::TransitionAndCancel
            && target_state == TradingState::Open
        {
            return Err(InvariantViolation::new(
                "transition-and-cancel observation targets the open state",
            ));
        }
        let selected_quantity_lots =
            try_order_selection_quantity(&orders, "trading-state control")?;
        Ok(Self {
            control,
            target_state,
            action,
    ...
    /// Returns the frozen current trading-state observation.
    #[must_use]
    pub const fn control(&self) -> TradingStateObservation {
        self.control
    }

    /// Returns the requested effective trading state.
    #[must_use]
    pub const fn target_state(&self) -> TradingState {
        self.target_state
    }

    /// Returns the requested state-control action.
    #[must_use]
    pub const fn action(&self) -> TradingStateControlAction {
        self.action
    }

    /// Returns the effective state produced by the requested action.
    #[must_use]
    pub const fn resulting_state(&self) -> TradingState {
        self.target_state
    }

    /// Returns complete selected states in ascending order-identifier order.
    ///
    /// Transition-only observations are empty. Transition-and-cancel
    /// observations contain every active resting or dormant-stop order.
    #[must_use]
    pub fn orders(&self) -> &[ActiveOrderSnapshot] {
        &self.orders
    }
...
impl OrderBook {
    ...
    /// Atomically observes and conditionally submits one complete new order.
    ///
    ...
    ///
    /// Market-to-limit observation and commit use the same current private
    /// executable best price. The quote describes current private execution
    /// economics; the submitted time-in-force remains authoritative for FOK,
    /// minimum-quantity IOC, post-only, and residual handling.
    ///
    /// # Errors
    ///
    ...
    /// Atomically observes and conditionally applies one account control.
    ///
    /// The predicate borrows the frozen current account fence and requested
    /// action. A block-and-cancel observation additionally contains every
    /// selected resting and dormant-stop state in canonical order under the
    /// same exclusive book borrow as commit; enable contains no selected
    /// orders and acquires no selection lease. Replay and core rejection bypass
    /// observation and predicate execution. Query failure, decline, or unwind
    /// precedes semantic mutation.
    ///
    /// # Errors
    ///
    /// Returns [`ConditionalOrderError::Matching`] for preparation or commit
    /// failure and [`ConditionalOrderError::Query`] when the current fence or
    /// complete caller-owned selected state cannot be validated or reserved.
    ...
    /// Atomically observes and conditionally applies one trading-state control.
    ///
    /// The predicate borrows the frozen current instrument state and requested
    /// target/action. A transition-and-cancel observation additionally contains
    /// every active resting and dormant-stop state in canonical order under the
    /// same exclusive book borrow as commit; transition-only contains no
    /// selected orders and acquires no selection lease. Replay and core
    /// rejection bypass observation and predicate execution. Query failure,
    /// decline, or unwind precedes semantic mutation.
    ///
    /// # Errors
    ///
    /// Returns [`ConditionalOrderError::Matching`] for preparation or commit
    /// failure and [`ConditionalOrderError::Query`] when the current state or
    /// complete caller-owned selected state cannot be validated or reserved.
    ...
    fn submit_conditional_account_control<T, E>(
        &mut self,
        command: AccountControl,
        observe: impl FnOnce(&Self, AccountControlSubmissionObservation) -> Result<T, E>,
        accept: impl FnOnce(&T) -> bool,
    ) -> Result<ConditionalCommandOutcome<T>, E>
    where
        E: From<MatchingError> + From<OrderBookQueryError>,
    ...
    fn submit_conditional_trading_state_control<T, E>(
        &mut self,
        command: TradingStateControl,
        observe: impl FnOnce(&Self, TradingStateControlSubmissionObservation) -> Result<T, E>,
        accept: impl FnOnce(&T) -> bool,
    ) -> Result<ConditionalCommandOutcome<T>, E>
    where
        E: From<MatchingError> + From<OrderBookQueryError>,
    ...
    where
        E: From<MatchingError>,
    {
        match evaluate_conditional_execution(preflight, |value| observe(self, value), accept)? {
        ...
        match self.prepare(Command::New(order))? {
            ...
            CommandPreparation::Ready(prepared) => {
                let observation = prepared
                    .core_rejection()
                    .is_none()
                    .then(|| self.new_order_execution_observation(order));
        ...
        E: From<MatchingError> + From<OrderBookQueryError>,
    {
            ...
            .map_err(E::from)?
        {
            ...
            CommandPreparation::Ready(mut prepared) => {
                let observation = if prepared.core_rejection().is_none() {
                    Some(
                        self.try_account_control_submission_observation(command, &mut prepared)
                            .map_err(E::from)?,
                    )
                } else {
                    None
        ...
        E: From<MatchingError> + From<OrderBookQueryError>,
    {
            ...
            .map_err(E::from)?
        {
            ...
            CommandPreparation::Ready(mut prepared) => {
                let observation = if prepared.core_rejection().is_none() {
                    Some(
                        self.try_trading_state_control_submission_observation(
                            command,
                            &mut prepared,
                        )
                        .map_err(E::from)?,
    ...
    fn try_mass_cancel_observation(
        ...
        let orders = self.try_selected_account_order_states(
            command.account_id,
            command.scope,
            prepared,
    ...
    pub(crate) fn try_account_control_submission_observation(
        &self,
        command: AccountControl,
        prepared: &mut PreparedCommand,
    ) -> Result<AccountControlSubmissionObservation, OrderBookQueryError> {
        let control = self.try_account_control_observation(command.account_id)?;
        if control.revision() != command.expected_revision {
            return Err(InvariantViolation::new(format!(
                "prepared account control expected revision {} but observed {}",
                command.expected_revision,
                control.revision()
            ))
            .into());
        }
        let orders = match command.action {
            AccountControlAction::BlockAndCancel => self.try_selected_account_order_states(
                command.account_id,
                MassCancelScope::All,
                prepared,
                OrderBookQueryResource::AccountControlOrders,
            )?,
            AccountControlAction::Enable => {
                if prepared.selected_order_ids.is_some() {
                    return Err(InvariantViolation::new(
                        "account enable preparation unexpectedly owns selection storage",
                    )
                    .into());
                }
                Vec::new()
        ...
        AccountControlSubmissionObservation::try_new(control, command.action, orders)
            .map_err(Into::into)
    }

    pub(crate) fn try_trading_state_control_submission_observation(
        &self,
        command: TradingStateControl,
        prepared: &mut PreparedCommand,
    ) -> Result<TradingStateControlSubmissionObservation, OrderBookQueryError> {
        let control = self.try_trading_state_observation()?;
        if control.revision() != command.expected_revision {
            return Err(InvariantViolation::new(format!(
                "prepared trading-state control expected revision {} but observed {}",
                command.expected_revision,
                control.revision()
            ))
            .into());
        ...
        let orders = match command.action {
            TradingStateControlAction::TransitionAndCancel => self.try_selected_order_states(
                prepared,
                OrderBookQueryResource::TradingStateControlOrders,
                OrderBook::try_select_all_orders_into,
            )?,
            TradingStateControlAction::Transition => {
                if prepared.selected_order_ids.is_some() {
                    return Err(InvariantViolation::new(
                        "transition-only trading-state preparation unexpectedly owns selection storage",
                    )
                    .into());
                }
                Vec::new()
        ...
        TradingStateControlSubmissionObservation::try_new(
            control,
    ...
    fn try_selected_account_order_states(
        ...
        self.try_selected_order_states(prepared, resource, |book, selected| {
            book.try_select_account_orders_into(account_id, scope, selected)
        })
    }

    fn try_selected_order_states(
        &self,
        prepared: &mut PreparedCommand,
        resource: OrderBookQueryResource,
    ...
    fn try_selected_values<T>(
        ...
        if !selected.values().is_empty() {
            return Err(InvariantViolation::new(
                "prepared order selection was populated more than once",
            )
            .into());
        }
    ...
    fn new_order_execution_observation(
        ...
        let constraint = match order.order_type {
            ...
            OrderType::MarketToLimit => StopActivation::Limit(
                self.best_price(order.side.opposite())
    ...
    /// Summarizes all public depth on one side without allocation or mutation.
    ///
    /// The selected range is the instrument definition's inclusive price
    /// domain. Hidden-only prices are absent. The result carries exact
    /// instrument/version/event-sequence provenance and market-priority
    /// best/worst selected prices.
    ///
    /// # Errors
    ///
    /// Returns [`InvariantViolation`] if public extrema are invalid or locked/
    ...
    /// Summarizes public depth inside one inclusive price range.
    ///
    /// The exact caller endpoints are retained in the fixed-size result. An
    /// inverted range is an empty summary. Bids are inspected from high to low
    /// and offers from low to high, so best/worst prices follow market priority.
    /// Fully hidden-only prices are absent. No successful path allocates or
    /// mutates the book.
    ///
    /// # Errors
    ///
    ...
    fn price_levels_range_iter(
        ...
        DirectionalIter::new(self.levels(side).range(range), side == Side::Buy)
    }
    ...
    fn apply_mass_cancel(
        ...
        let (cancelled_order_count, cancelled_quantity_lots) = self.cancel_account_orders(
            command.account_id,
            command.scope,
            command.command_id,
            command.received_at,
    ...
    #[allow(
        clippy::too_many_arguments,
        reason = "the shared account cancellation primitive carries command/event provenance"
    )]
    fn cancel_account_orders(
        &mut self,
        account_id: AccountId,
        scope: MassCancelScope,
        command_id: CommandId,
        ...
        reason: CancelReason,
        events: &mut EventTraceBuilder,
        selected: &mut PooledOrderSelection,
    ) -> Result<(u64, u128), MatchingError> {
        let selected_count = self.account_active_order_count(account_id, scope);
        ...
        if selected_values.is_empty() {
            self.try_select_account_orders_into(account_id, scope, selected_values)
                .map_err(|_| MatchingError::InternalInvariantViolation)?;
        } else {
            self.try_validate_selected_account_orders(account_id, scope, selected_values)
                .map_err(|_| MatchingError::InternalInvariantViolation)?;
        }
        debug_assert_eq!(selected_values.len(), selected_count);
        debug_assert_eq!(selected_values.as_ptr(), selected_buffer);
        self.detach_account_order_scope(account_id, scope, selected_count);
        ...
        Ok((cancelled_order_count, cancelled_quantity_lots))
    }
    ...
    fn cancel_all_orders(
        ...
        let selected_buffer = selected_values.as_ptr();
        if selected_values.is_empty() {
            self.try_select_all_orders_into(selected_values)
                .map_err(|_| MatchingError::InternalInvariantViolation)?;
        } else {
            self.try_validate_selected_all_orders(selected_values)
                .map_err(|_| MatchingError::InternalInvariantViolation)?;
        }
        debug_assert_eq!(selected_values.len(), selected_count);
        debug_assert_eq!(selected_values.as_ptr(), selected_buffer);
    ...
    fn try_select_all_orders_into(
        &self,
        selected: &mut Vec<OrderId>,
    ) -> Result<(), InvariantViolation> {
        if !selected.is_empty() {
            return Err(InvariantViolation::new(
                "all-order selection output is not empty",
            ));
        }
        let selected_count = self.orders.len();
        if selected.capacity() < selected_count {
            return Err(InvariantViolation::new(format!(
                "all-order selection capacity {} is below required count {selected_count}",
                selected.capacity()
            )));
        }
        ...
        if selected.len() != selected_count {
            return Err(InvariantViolation::new(format!(
                "all-order selection length {} differs from active count {selected_count}",
                selected.len()
            )));
        }
        selected.sort_unstable();
        if selected.windows(2).any(|pair| pair[0] == pair[1]) {
            return Err(InvariantViolation::new(
                "all-order selection contains a duplicate order",
            ));
        }
        Ok(())
    }
    ...
    fn try_validate_selected_all_orders(
        ...
        let expected_count = self.orders.len();
        if selected.len() != expected_count {
            return Err(InvariantViolation::new(format!(
                "prepared all-order selection length {} differs from current count {expected_count}",
                selected.len()
            )));
        }
        let mut previous_order_id = None;
        for &order_id in selected {
            if previous_order_id.is_some_and(|previous| previous >= order_id) {
                return Err(InvariantViolation::new(format!(
                    "prepared all-order selection is not strictly ordered at order {order_id}"
                )));
            }
            previous_order_id = Some(order_id);
            if !self.orders.contains_key(&order_id) {
                return Err(InvariantViolation::new(format!(
                    "prepared all-order selection references absent order {order_id}"
                )));
            }
        }
        Ok(())
...
mod price_level_index_tests {
    ...
    #[test]
    fn depth_summary_rejects_invalid_or_overflowing_public_aggregates() {
        let mut book = OrderBook::new(reserve_definition());
        book.bids.insert(Price::from_raw(100), level(1));
        ...
        book.bids
            ...
            .unwrap();
        let error = book.try_depth_summary(Side::Buy).unwrap_err();
        assert!(error.detail().contains("zero aggregate or order count"));
    ...
    #[test]
    fn conditional_account_control_propagates_selection_corruption_and_returns_its_lease() {
        let mut book = OrderBook::new(reserve_definition());
        let account_id = AccountId::new(1).unwrap();
        for (command_id, order_id, price) in [(1_u64, 1_u64, 100_i64), (2, 2, 99)] {
            book.submit(Command::New(NewOrder {
        ...
        assert_eq!(book.retained_command_count(), 2);
        assert_eq!(book.active_order_count(), 2);
        assert_eq!(book.account_control(account_id).revision(), 0);
        assert_eq!(book.order_selection_pool_status(), pool_before);
    }

    #[test]
    fn conditional_trading_state_control_propagates_order_corruption_and_returns_its_lease() {
        let mut book = OrderBook::new(reserve_definition());
        for (command_id, order_id, price) in [(1_u64, 1_u64, 100_i64), (2, 2, 99)] {
            book.submit(Command::New(NewOrder {
                command_id: CommandId::new(command_id).unwrap(),
        ...
        let error = book
            .try_submit_trading_state_control_if(
                ...
                |_| panic!("order corruption must bypass the predicate"),
            )
            .unwrap_err();

        let ConditionalOrderError::Query(OrderBookQueryError::InvariantViolation(error)) = error
        else {
            panic!("order corruption must remain a typed query failure");
        };
        assert!(
            error
                .detail()
                .contains("order observation key 1 resolves to order 3"),
            "{}",
            error.detail()
        );
        assert_eq!(book.last_event_sequence(), sequence_before);
