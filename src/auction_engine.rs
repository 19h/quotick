...
/// Exact owned state offered to one conditional indicative publication.
///
/// The observation binds the prepared publication and previous still-current
/// indication to one engine, phase, book, history, and sequence generation. It
/// is fixed-size, allocation-free, and remains usable after decline or commit.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CallAuctionIndicativeObservation {
    ...
    published_indicative: CallAuctionIndicativeState,
}

impl CallAuctionIndicativeObservation {
    /// Returns the exact command identity.
    #[must_use]
    pub const fn command_id(&self) -> CommandId {
        self.command_id
    ...
    /// Returns the publication event sequence assigned by acceptance.
    #[must_use]
    pub const fn first_event_sequence(&self) -> u64 {
        self.first_event_sequence
    }

    /// Returns the exact phase and cycle generation used by discovery.
    #[must_use]
    pub const fn phase(&self) -> CallAuctionPhaseSnapshot {
        self.phase
    }

    /// Returns the collection-book revision used by discovery.
    #[must_use]
    pub const fn book_revision(&self) -> u64 {
        self.published_indicative.book_revision()
    }

    /// Returns the unchanged collection-book revision after publication.
    #[must_use]
    pub const fn resulting_book_revision(&self) -> u64 {
        self.published_indicative.book_revision()
    }
    ...
    /// Returns the previous still-current publication, if present.
    #[must_use]
    pub const fn previous_indicative(&self) -> Option<CallAuctionIndicativeState> {
        self.previous_indicative
            .expand(self.phase, self.published_indicative.book_revision())
    }

    /// Returns the exact state published by acceptance.
    #[must_use]
    pub const fn published_indicative(&self) -> CallAuctionIndicativeState {
        self.published_indicative
    }
}

/// Exact owned state offered to one conditional call-auction submission.
///
/// The observation is bound to one same-generation preparation and contains
/// the exact active state admitted by acceptance. It is fixed-size,
/// allocation-free, and remains usable after either decline or accepted commit.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CallAuctionSubmitObservation {
    command_id: CommandId,
    instrument_id: InstrumentId,
    instrument_version: InstrumentVersion,
    command_sequence: u64,
...
impl CallAuctionSubmitObservation {
    /// Returns the exact command identity.
    #[must_use]
    pub const fn command_id(self) -> CommandId {
        self.command_id
    ...
    /// Returns the admission event sequence assigned by acceptance.
    #[must_use]
    pub const fn first_event_sequence(self) -> u64 {
        self.first_event_sequence
    }
    ...
    /// Returns the owner of the admitted order.
    #[must_use]
    pub const fn account_id(self) -> AccountId {
        self.accepted.account_id
    }

    /// Returns the admitted order identifier.
    #[must_use]
    ...
    /// Returns the exact active order state produced by acceptance.
    #[must_use]
...
/// Exact zero-copy state offered to one conditional account mass cancellation.
///
/// The value borrows constructor-owned scratch containing the canonical active
/// orders selected by the same-generation preparation. It is valid only during
/// the synchronous predicate call and cannot outlive or mutate that selection.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CallAuctionMassCancelObservation<'a> {
    command_id: CommandId,
    instrument_id: InstrumentId,
    instrument_version: InstrumentVersion,
    command_sequence: u64,
    ...
    phase: CallAuctionPhaseSnapshot,
    book_revision: u64,
    result: CallAuctionMassCancelResult,
    received_at: TimestampNs,
    previous_indicative: CompactOptionalCallAuctionIndicativeState,
    account_id: AccountId,
    scope: MassCancelScope,
    orders: &'a [CallAuctionOrderSnapshot],
}

impl CallAuctionMassCancelObservation<'_> {
    /// Returns the exact command identity.
    #[must_use]
    pub const fn command_id(&self) -> CommandId {
        self.command_id
    ...
    /// Returns the first cancellation event sequence, or the completion event
    /// sequence when the selection is empty.
    #[must_use]
    pub const fn first_event_sequence(&self) -> u64 {
        self.first_event_sequence
    }

    /// Returns the mass-cancel completion event sequence.
    #[must_use]
    pub const fn completion_event_sequence(&self) -> u64 {
        self.first_event_sequence
            .wrapping_add(self.result.cancelled_order_count())
    }
    ...
    /// Returns the exact collection-book revision used by selection.
    #[must_use]
    pub const fn book_revision(&self) -> u64 {
        self.book_revision
    }
    ...
    /// Returns the exact number of selected active orders.
    #[must_use]
    pub const fn cancelled_order_count(&self) -> u64 {
        self.result.cancelled_order_count()
    }

    /// Returns the exact selected active quantity in instrument-defined lots.
    #[must_use]
    pub const fn cancelled_quantity_lots(&self) -> u128 {
        self.result.cancelled_quantity_lots()
    }
    ...
    #[must_use]
    pub const fn orders(&self) -> &[CallAuctionOrderSnapshot] {
        self.orders
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct CallAuctionMassCancelObservationContext {
...
impl CallAuctionMassCancelObservationContext {
    const fn observe(
        self,
        orders: &[CallAuctionOrderSnapshot],
    ) -> CallAuctionMassCancelObservation<'_> {
        CallAuctionMassCancelObservation {
            command_id: self.command.command_id,
            instrument_id: self.command.instrument_id,
            instrument_version: self.command.instrument_version,
            ...
            previous_indicative: self.previous_indicative,
            account_id: self.command.account_id,
...
struct CallAuctionMassCancelScratch<'a> {
    orders: &'a mut Vec<CallAuctionOrderSnapshot>,
}

impl Drop for CallAuctionMassCancelScratch<'_> {
    fn drop(&mut self) {
        self.orders.clear();
    }
}

fn try_mass_cancel_observation_context(
    engine: &CallAuctionEngine,
    prepared: &PreparedCallAuctionCommand,
) -> Result<CallAuctionMassCancelObservationContext, CallAuctionEngineError> {
    if prepared.engine_instance_id != engine.instance_id {
        return Err(CallAuctionEngineError::ForeignPreparation);
    }
    if prepared.expected_retained_commands != engine.reports.len()
        || prepared.expected_retained_events != engine.retained_event_count
    {
        return Err(CallAuctionEngineError::StalePreparation);
    }
    let CallAuctionCommand::MassCancel(command) = prepared.command else {
        return Err(CallAuctionEngineError::InternalInvariantViolation);
    };
    let PreparedCallAuctionAction::MassCancel {
        account_id,
        scope,
        book_revision,
        preflight,
    ...
    let current = engine
        .book
        .preflight_mass_cancel(command.account_id, command.scope)
        .map_err(CallAuctionEngineError::BookMassCancellation)?;
    engine
        .next_event_sequence
        .checked_add(preflight.cancelled_order_count())
        .ok_or(CallAuctionEngineError::InternalInvariantViolation)?;
    if account_id != command.account_id
        || scope != command.scope
        || current != preflight
        || engine.book.definition().instrument_id() != command.instrument_id
        || engine.book.definition().version() != command.instrument_version
        || (engine.phase == CallAuctionPhase::Closed) != engine.active_auction_id.is_none()
        || (engine.active_auction_id.is_some()
            && engine.active_auction_id != engine.last_auction_id)
        || engine.last_indicative.is_some_and(|indicative| {
            engine.active_auction_id != Some(indicative.auction_id())
                || indicative.phase_revision() != engine.phase_revision
                || indicative.book_revision() != book_revision
                || !indicative.is_valid_for(engine.book.definition())
        })
    {
        return Err(CallAuctionEngineError::InternalInvariantViolation);
    }
    Ok(CallAuctionMassCancelObservationContext {
        command,
        command_sequence: engine.next_command_sequence,
        first_event_sequence: engine.next_event_sequence,
...
pub(crate) fn evaluate_conditional_mass_cancel(
    ...
    match preparation {
        ...
        CallAuctionCommandPreparation::Ready(prepared) => {
            ...
            let context = try_mass_cancel_observation_context(engine, &prepared)?;
            debug_assert!(engine.mass_cancel_scratch.is_empty());
            let result = engine
                .book
                .preflight_mass_cancel_into(
                    context.command.account_id,
                    context.command.scope,
                    &mut engine.mass_cancel_scratch,
                )
                .map_err(CallAuctionEngineError::BookMassCancellation)?;
            if result != context.result {
                engine.mass_cancel_scratch.clear();
                return Err(CallAuctionEngineError::InternalInvariantViolation);
            }
            let accepted = {
                let scratch = CallAuctionMassCancelScratch {
                    orders: &mut engine.mass_cancel_scratch,
                };
                let observation = context.observe(scratch.orders.as_slice());
                accept(&observation)
            };
            if accepted {
...
pub(crate) fn evaluate_conditional_submit(
    engine: &CallAuctionEngine,
    preparation: CallAuctionCommandPreparation,
    observe_authorized: bool,
    accept: impl FnOnce(&CallAuctionSubmitObservation) -> bool,
) -> Result<
    ConditionalCallAuctionCommandPreparation<CallAuctionSubmitObservation>,
...
pub(crate) fn evaluate_conditional_indicative(
    engine: &CallAuctionEngine,
    preparation: CallAuctionCommandPreparation,
    accept: impl FnOnce(&CallAuctionIndicativeObservation) -> bool,
) -> Result<
    ConditionalCallAuctionCommandPreparation<CallAuctionIndicativeObservation>,
    CallAuctionEngineError,
> {
    evaluate_conditional_call_auction_command(
        engine,
        preparation,
        true,
        CallAuctionEngine::try_indicative_observation,
        accept,
    )
}
...
impl CallAuctionEngine {
    ...
    /// state containing the exact order identity and priority assigned by
    /// acceptance. Decline or unwind drops the preparation without sequencing
    /// or mutation; acceptance commits that same preparation and returns the
    /// exact observation with its report.
    ///
    /// # Errors
    ///
    ...
    > {
        ...
        match evaluate_conditional_submit(self, preparation, true, accept)? {
            ConditionalCallAuctionCommandPreparation::Complete(outcome) => Ok(outcome),
            ConditionalCallAuctionCommandPreparation::Commit {
                prepared,
                observation,
    ...
    /// Atomically observes and conditionally commits one account mass cancel.
    ///
    /// Replay and deterministic business rejection bypass `accept`. A
    /// core-admissible command invokes the predicate with a zero-copy view of
    /// the exact canonical selection in constructor-owned scratch. Decline or
    /// unwind clears that scratch without sequencing or mutation; acceptance
    /// commits the same-generation preparation.
    ///
    /// # Errors
    ///
    /// Returns [`CallAuctionEngineError`] for preparation, fail-closed
    /// selection, generation validation, or commit failure.
    pub fn try_submit_mass_cancel_if(
        ...
        match evaluate_conditional_mass_cancel(self, preparation, accept)? {
    ...
    /// Atomically observes and conditionally publishes one indicative state.
    ///
    /// Replay and deterministic business rejection bypass `accept`. A
    /// core-admissible command invokes the predicate with the exact prepared
    /// clearing state, prior publication, and phase/book/sequence provenance.
    /// Decline or unwind changes no sequence, publication, book, or history;
    /// acceptance publishes the same state in one canonical event.
    ///
    /// # Errors
    ///
    /// Returns [`CallAuctionEngineError`] for preparation, fail-closed
    ...
    > {
        ...
        match evaluate_conditional_indicative(self, preparation, accept)? {
            ConditionalCallAuctionCommandPreparation::Complete(outcome) => Ok(outcome),
            ConditionalCallAuctionCommandPreparation::Commit {
                prepared,
                observation,
    ...
    fn try_submit_observation(
        &self,
        prepared: &PreparedCallAuctionCommand,
    ) -> Result<CallAuctionSubmitObservation, CallAuctionEngineError> {
        ...
        let CallAuctionCommand::Submit(command) = prepared.command else {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
        };
        let PreparedCallAuctionAction::Submit {
            accepted,
            book_revision,
        } = prepared.action
        else {
        ...
        let predicted = self
            .book
            .preflight_admission(command.order)
            .map_err(|_| CallAuctionEngineError::InternalInvariantViolation)?;
        let identity = self
            .book
            .try_order_observation(command.order.order_id())
            .map_err(CallAuctionEngineError::BookQuery)?;
        book_revision
            .checked_add(1)
            .ok_or(CallAuctionEngineError::InternalInvariantViolation)?;
        if predicted != accepted
            || identity.instrument_id() != command.order.instrument_id()
            || identity.instrument_version() != command.order.instrument_version()
            || identity.book_revision() != book_revision
            || identity.order_id() != command.order.order_id()
            ...
            || self.last_auction_id != self.active_auction_id
        {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
        }
        if self.last_indicative.is_some_and(|indicative| {
        ...
        Ok(CallAuctionSubmitObservation {
            command_id: command.command_id,
            instrument_id: command.order.instrument_id(),
            instrument_version: command.order.instrument_version(),
    ...
    fn try_indicative_observation(
        &self,
        prepared: &PreparedCallAuctionCommand,
    ) -> Result<CallAuctionIndicativeObservation, CallAuctionEngineError> {
        if prepared.engine_instance_id != self.instance_id {
            return Err(CallAuctionEngineError::ForeignPreparation);
        }
        if prepared.expected_retained_commands != self.reports.len()
        ...
        let CallAuctionCommand::Indicative(command) = prepared.command else {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
        };
        let PreparedCallAuctionAction::Indicative { state, phase } = prepared.action else {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
        };
        self.validate_prepared_indicative(command, state, phase)?;
        Ok(CallAuctionIndicativeObservation {
            command_id: command.command_id,
            ...
            published_indicative: state,
        })
    }

    fn validate_prepared_indicative(
        &self,
        command: CallAuctionIndicativeCommand,
        state: CallAuctionIndicativeState,
        phase: CallAuctionPhaseSnapshot,
        ...
        if self.book.definition().instrument_id() != command.instrument_id
            ...
            || phase.revision() != command.expected_phase_revision
            || phase.active_auction_id() != Some(command.auction_id)
            ...
            || !state.is_valid_for(self.book.definition())
            || self.last_indicative.is_some_and(|indicative| {
                indicative.auction_id() != command.auction_id
                    || indicative.phase_revision() != phase.revision()
                    || indicative.book_revision() != state.book_revision()
                    || !indicative.is_valid_for(self.book.definition())
            })
        {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
    ...
    fn apply_prepared_action(
        ...
        let invalidates_indicative = !matches!(
            &action,
            PreparedCallAuctionAction::Rejected(_) | PreparedCallAuctionAction::Indicative { .. }
        );
        let outcome = match action {
            ...
            PreparedCallAuctionAction::Submit {
                accepted,
                book_revision,
            } => self.apply_submit(command, accepted, book_revision, events)?,
            ...
            PreparedCallAuctionAction::Indicative { state, phase } => {
                self.apply_indicative(command, state, phase, events)?
            }
    ...
    fn apply_submit(
        ...
        let predicted = self
            .book
            .preflight_admission(submit.order)
            .map_err(|_| CallAuctionEngineError::InternalInvariantViolation)?;
        if predicted != accepted {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
        }
        ...
        self.push_event(
            command,
            CallAuctionEventKind::OrderAccepted(admitted),
            events,
        );
        Ok(CallAuctionCommandOutcome::Accepted)
    }
    ...
    fn apply_indicative(
        ...
        let CallAuctionCommand::Indicative(indicative) = command else {
            return Err(CallAuctionEngineError::InternalInvariantViolation);
        };
        self.validate_prepared_indicative(indicative, state, phase)?;
...
mod tests {
    ...
    use super::{
        ...
        CallAuctionEventKind, CallAuctionIndicativeCommand, CallAuctionMassCancel,
    ...
    fn crossed_collecting_engine() -> CallAuctionEngine {
        ...
        engine
            ...
            .unwrap();
        for (command_id, order_id, account_id, side, price) in [
            (2_u64, 1_u64, 1_u64, Side::Buy, 60_i64),
            (3, 2, 2, Side::Sell, 40),
        ] {
            engine
    ...
    fn indicative_command(engine: &CallAuctionEngine) -> CallAuctionIndicativeCommand {
    ...
    #[test]
    fn conditional_submit_rejects_a_stale_book_preparation_before_predicate() {
        let mut engine = CallAuctionEngine::try_new(definition()).unwrap();
        let instrument_id = engine.book.definition().instrument_id();
        let instrument_version = engine.book.definition().version();
        let auction_id = AuctionId::new(1).unwrap();
        ...
        engine
            ...
            .unwrap();
        let preparation = engine
            .prepare(CallAuctionCommand::Submit(CallAuctionSubmitOrder {
        ...
        engine
            ...
            .unwrap();
        let called = Cell::new(false);
        let error = evaluate_conditional_submit(&engine, preparation, true, |_| {
            called.set(true);
            true
        })
        .unwrap_err();
        ...
        assert!(!called.get());
        assert!(matches!(error, CallAuctionEngineError::StalePreparation));
        assert!(engine.book.order(OrderId::new(2).unwrap()).is_none());
    }

    #[test]
    fn conditional_indicative_rejects_a_stale_book_before_predicate() {
        let mut engine = crossed_collecting_engine();
        let command = indicative_command(&engine);
        let preparation = engine
        ...
        engine
            ...
            .unwrap();
        let called = Cell::new(false);
        let error = evaluate_conditional_indicative(&engine, preparation, |_| {
            called.set(true);
            true
        })
        .unwrap_err();
    ...
    #[test]
    fn indicative_commit_rejects_a_book_change_after_predicate_acceptance() {
        let mut engine = crossed_collecting_engine();
        let command = indicative_command(&engine);
        let preparation = engine
            .prepare(CallAuctionCommand::Indicative(command))
        ...
        let ConditionalCallAuctionCommandPreparation::Commit {
            prepared,
            observation: Some(observation),
        } = evaluate_conditional_indicative(&engine, preparation, |_| true).unwrap()
        else {
            panic!("core-admissible indicative publication must be commit-ready");
        };
    ...
    #[test]
    fn conditional_mass_cancel_rejects_a_stale_book_preparation_before_predicate() {
        let mut engine = CallAuctionEngine::try_new(definition()).unwrap();
        let instrument_id = engine.book.definition().instrument_id();
        let instrument_version = engine.book.definition().version();
        let auction_id = AuctionId::new(1).unwrap();
        engine
            ...
            .unwrap();
        for (command_id, order_id, account_id) in [(2_u64, 1_u64, 1_u64), (3, 2, 2)] {
            engine
                .submit(CallAuctionCommand::Submit(CallAuctionSubmitOrder {
                    command_id: CommandId::new(command_id).unwrap(),
                    auction_id,
        ...
        let preparation = engine
            .prepare(CallAuctionCommand::MassCancel(CallAuctionMassCancel {
                command_id: CommandId::new(4).unwrap(),
                instrument_id,
                instrument_version,
                account_id: AccountId::new(1).unwrap(),
        ...
        engine
            ...
            .unwrap();
        let called = Cell::new(false);
        let error = evaluate_conditional_mass_cancel(&mut engine, preparation, |_| {
            called.set(true);
            true
        })
        .unwrap_err();
        ...
        assert!(matches!(error, CallAuctionEngineError::StalePreparation));
        assert!(engine.book.order(OrderId::new(1).unwrap()).is_some());
        assert!(engine.mass_cancel_scratch.is_empty());
    }
